    ...
    const fetchAllInitialData = async () => {
      ...
      try {
        const [productCollectionsRes, categoryUrlsRes, currencyRes] = await Promise.all([
          ...
          fetch("http://localhost:3000/api/payload/collections?sort=order").then(res => res.ok ? res.json() : Promise.reject(`Product Collections fetch failed: ${res.status}`)),
          // Fetch category URLs from /api/payload/categories
          fetch("http://localhost:3000/api/payload/categories").then(res => res.ok ? res.json() : Promise.reject(`Category URLs fetch failed: ${res.status}`)),
