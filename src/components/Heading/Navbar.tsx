...
import React, { useState, useEffect, useRef, forwardRef, ChangeEvent, FormEvent, KeyboardEvent } from 'react';
...
// EnhancedSearchInput and SearchModal components remain unchanged from your provided code...
// ... (EnhancedSearchInput component code as provided)
// ... (SearchModal component code as provided)
// For brevity, I'm omitting the EnhancedSearchInput and SearchModal here as they are not directly related to the mobile menu opening animation.
// Paste your EnhancedSearchInput and SearchModal components here.

const EnhancedSearchInput = forwardRef < HTMLInputElement,
    {
    ...
    } > (({ onChange, onSubmit, onClear, onSuggestionSelect, value, searchSuggestions, }, ref) => {
        ...
        const filteredSuggestions = value ?
            searchSuggestions.filter((s) => s.toLowerCase().includes(value.toLowerCase())).slice(0, 5) :
            [];
...
const SearchModal = ({
    ...
    const fetchQuickLinks = async () => {
        ...
        const quickLinksApiUrl = 'http://localhost:3000/api/search-suggestions';
        const fallbackLinks = ['New Arrivals', 'Best Sellers', 'Sale', 'Collections'];
        try {
            ...
            if (Array.isArray(data?.suggestions)) {
                setQuickLinks(data.suggestions.slice(0, 8));
            } else {
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    className="fixed inset-0 bg-black/90 backdrop-blur-lg flex items-start lg:items-center justify-center z-50 px-4 pt-16 lg:pt-4 pb-4"
                >
                    <motion.div
                        ref={modalRef}
                        ...
                        className="bg-neutral-900/70 backdrop-blur-xl border border-neutral-800 rounded-3xl w-full max-w-[640px] p-6 md:p-8 shadow-2xl relative overflow-hidden max-h-[calc(100vh-5rem)] lg:max-h-[90vh] flex flex-col"
                    >
                        ...
                        <motion.div
                            ...
                            className="mb-5 flex items-center flex-shrink-0"
                        >
                        ...
                        <div className="overflow-y-auto flex-grow min-h-0 pr-1 mr-[-4px]">
                            {!searchQuery && (
                                <>
                                    <motion.div
                                                ...
                                                {recentSearches.map((search, index) => (
                                                    <motion.li
                                                        ...
                                                        className={cn(
                                                            ...
                                                            focusedRecent === index ?
                                                            'bg-neutral-800/70 text-white ring-1 ring-neutral-700' :
                                                            'text-neutral-300'
                                                        )}
                                            ...
                                            <div className="grid grid-cols-2 gap-3">
                                                {quickLinks.map((category, index) => (
                                                    <motion.button
                                                        ...
                                                        whileTap={{ scale: 0.98 }}
                                                        onClick={() => handleSubmitForm(category)}
                                                    >
                        ...
                        <motion.button
                            ...
                            onClick={onClose}
                            type="button"
                            className="absolute top-5 right-5 md:top-6 md:right-6 bg-neutral-800/40 hover:bg-neutral-700/50 p-2.5 rounded-full transition-colors flex-shrink-0 z-20"
...
const Navbar = () => {
    ...
    return (
        <div className="relative w-full text-neutral-400">
            ...
            {/* MainNavbarHeader - FIXED HEIGHT, TRANSFORM REMOVED/MODIFIED */}
            <header
                className={cn(
                    'fixed top-4 inset-x-0 mx-auto max-w-6xl px-2 md:px-12 z-[100]',
                    // ---
                    // KEY CHANGE HERE: Header no longer changes height.
                    // Removed 'transform transition-all duration-300 ease-out' which might interfere
                    // with fixed children if not desired, and was likely for height animation.
                    // If other transforms are needed on the header, they should be added explicitly.
                    // ---
                    'h-12' // Standard, fixed height for the header bar
                )}
            >
                {/* Wrapper: applies background, border, rounded corners */}
                <Wrapper
                    className={cn(
                        "relative backdrop-blur-lg backdrop-brightness-75 rounded-xl lg:rounded-3xl border border-[rgba(124,124,124,0.2)] px-1 md:px-2 flex items-center justify-start h-full"
                        // If header height is fixed (e.g. h-12), Wrapper's h-full will respect that.
                    )}
                >
                    {/* HeaderContentRow - Adjusted vertical alignment */}
                    <div className="flex items-center justify-between w-full"> {/* Removed sticky and auto margins as header height is fixed */}
                    ...
                    {/* MobileMenuComponent */}
                    {/* Render only on mobile screens. Positioned inside Wrapper, so MobileMenu's own fixed positioning will be key. */}
                    {!isLargeScreen && (
                        <MobileMenu
                            isOpen={isOpen}
                            setIsOpen={setIsOpen}
